...
from head_controller.llm_secret_sanitizer import dumps_sanitized, sanitize_for_report
...
class HeadControllerHTMLReportV556:
    def build(self, reports_dir: str | Path = "docs/reports", llm_provider: str = "off", key_file: str | None = None, docs_output_dir: str | Path | None = None, replay_output_dir: str | Path | None = None) -> Path:
        ...
        out = Path(replay_output_dir) if replay_output_dir else production_replay_report_dir("head_controller_v556")
        ...
        (out / "head_controller_report.json").write_text(dumps_sanitized(summary), encoding="utf-8")
        ...
        (docs_reports_dir / "latest_head_controller_draft_summary.json").write_text(dumps_sanitized(summary), encoding="utf-8")
