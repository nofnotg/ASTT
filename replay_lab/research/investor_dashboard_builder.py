...
def build_investor_dashboard(reports_dir: str = "docs/reports") -> dict[str, str]:
    ...
    html = _dashboard_html(summary, capital, archive)
...
def _dashboard_html(summary: dict[str, Any], capital: dict[str, Any], archive: dict[str, Any]) -> str:
...
<head>
  ...
  <style>
    ...
    li{{margin:10px 0}}code{{background:#e2e8f0;padding:2px 5px;border-radius:4px}}
  </style>
</head>
<body>
  <header>
    <h1>ASTT 투자 리포트 대시보드</h1>
    <p>모든 PAPER 투자일지, 주간/월간 결과, 리스크 분석을 모아보는 고정 페이지입니다.</p>
  </header>
  <main>
    ...
    <section class="grid">
      ...
      <div class="card"><div>최종 계좌 평가금(Equity)</div><div class="kpi good">{final_equity:,.0f} KRW</div></div>
      <div class="card"><div>총수익률(Return)</div><div class="kpi {'good' if ret >= 0 else 'bad'}">{ret:+.2f}%</div></div>
      <div class="card"><div>최대 낙폭(MDD)</div><div class="kpi warn">{mdd:.2f}%</div></div>
      <div class="card"><div>거래 수</div><div class="kpi">{trades}</div></div>
      <div class="card"><div>현재 판정</div><div class="kpi bad">{judgement}</div></div>
    </section>
    <h2>최신 리포트 바로 보기</h2>
    <ul>
      ...
      <li><a href="latest_head_controller_v62_review_report.html">Head Controller 검토</a> - 다음 실험과 실전 금지 사유</li>
    </ul>
    <h2>용어 설명</h2>
    <table>
      ...
      <tr><td>거래당 기대수익(Expectancy)</td><td>거래 한 번을 할 때 평균적으로 기대할 수 있는 수익 또는 손실입니다.</td></tr>
      <tr><td>가격 공백 구간(FVG)</td><td>가격이 빠르게 움직이며 비어 보이는 구간입니다. 되돌림 반응 후보로 봅니다.</td></tr>
      <tr><td>유동성 훑기/휩쏘(Liquidity Sweep)</td><td>전저점/전고점을 잠깐 깨고 다시 회복하는 속임수성 움직임입니다.</td></tr>
