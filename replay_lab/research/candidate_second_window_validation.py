...
def validate_candidate_second_windows(start_date, end_date, top_markets: int = 30, pre_seconds: int = 120, post_seconds: int = 180, fixed_order_krw: float = 10000, max_candidates: int = 50) -> dict:
...
def _metrics(rows: list[dict]) -> dict:
    entered = [r for r in rows if r.get("entry_decision") != "CANCEL"]
    pnl = [float(r.get("net_pnl_pct", 0.0)) for r in entered]
    gross = [float(r.get("gross_pnl_pct", 0.0)) for r in entered]
    wins = [x for x in pnl if x > 0]
    losses = [x for x in pnl if x < 0]
    gwins = [x for x in gross if x > 0]
    glosses = [x for x in gross if x < 0]
    return {"entry_count": len(entered), "cancel_count": len(rows) - len(entered), "win_rate": len(wins) / len(entered) if entered else 0.0, "profit_factor_gross": sum(gwins) / abs(sum(glosses)) if glosses else (999.0 if gwins else 0.0), "profit_factor_realistic_1": sum(wins) / abs(sum(losses)) if losses else (999.0 if wins else 0.0), "expectancy_realistic_1": sum(pnl) / len(pnl) if pnl else 0.0, "total_pnl_krw_realistic_1": sum(float(r.get("net_pnl_krw", 0.0)) for r in entered), "avg_hold_seconds": sum(float(r.get("hold_seconds", 0.0)) for r in entered) / len(entered) if entered else 0.0, "micro_failure_exit_count": sum(1 for r in entered if r.get("exit_reason") == "MICRO_FAILURE_EXIT"), "time_stop_count": sum(1 for r in entered if r.get("exit_reason") == "TIME_STOP")}
